...
//sync in js
   // synchronous:
       ...
       // each instruction wait for the previous instructionn to complete its execution
...
// syncchronus i.e, 1st executed 1st then 2nd statement and then 3rd (all prog we did upto now are synchronous in js)
   ...
   // Asynchronous:
       ...
       // if a statemnt take more time to exe the remaining code will run with parrallel and don't wait to complete the exe, and as the time tacker code exe completed it will show its output



// setTimeout(call, time in mS):    // we set time for the func we pass that exe it after the time  // 2 Sec === 2000 msec
function hello() {
...
console.log("after");   // this tatement will not wait for the above func to exe as it doesn't defends on yhe above funx result, it will exe in paralell with the func and the output of the func will showed after the statement(as it executed after)
/////ouput:
//before
//after
// Hey! Jamal...    // after 4s
...
function calcCallback(a,b,sumCallback) {    
   sum(a,b)
}
calcCallback(2,5,sum)       // in callback we oassed func without parenthesis b\c passing with parenthesis means that we exe it here and we wanna exe it inside the another func as in line 64 

function callc(1,2,(a,b)=>{
    console.log(a+b);
});
